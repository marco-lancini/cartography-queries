...
class Client:
    ...
    def send(self, index, doc_type, data):
        ...
        if not (res.get('created') or res.get('result') == 'created'):
            raise Exception('Failed to submit to Elasticsearch: created:{} result:{}'.format(res.get('created'), res.get('result')))
...
class ElasticsearchConsumer(object):
    ...
    def _connect(self):
        ...
        self.es_client = Client(url=self.es_location,
                                ca=certifi.where(),
                                dry_run=self.dry_run)
        logger.info('ElasticSearch Client instantiated: {} / {}'.format(self.url, self.index))
